        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10 mb-16">
          ...
          <div className="sm:col-span-2">
            ...
            <div className="flex flex-col gap-3">
              <a href="mailto:partnership@superstart.in" className="flex items-center gap-3 text-sm text-white/60 hover:text-white transition">
                <HiOutlineMail className="w-4 h-4" />
                partnership@superstart.in
              </a>

              <a href="tel:+917414974582" className="flex items-center gap-3 text-sm text-white/60 hover:text-white transition">
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category}>
              ...
              <ul className="flex flex-col gap-2">
                {links.map((link) => (
                  <li key={link.label}>
                    <a href={link.href} className="text-white/60 hover:text-white text-sm transition">
                      {link.label}
                    </a>
                  </li>
                ))}
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 md:p-8 mb-12 flex flex-col md:flex-row gap-6 items-start md:items-center">
          ...
          <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row w-full md:w-auto gap-3">
            <input
              ...
              className="w-full sm:w-64 px-4 py-3 rounded-lg bg-white/5 border border-white/10 text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-white/20"
            />

            <button
              type="submit"
              disabled={loading}
              className="px-6 py-3 rounded-lg bg-white text-[#020617] text-sm font-medium hover:opacity-90 transition w-full sm:w-auto disabled:opacity-50"
            >
